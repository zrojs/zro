import { auth, logout } from "@zro/auth";
...
export const loader = async () => {
  const orm = getOrm();
  return {
    tasks: await orm.select().from(tasks).all(),
  };
};
...
function CreateTaskInput() {
  const addTaskAction = useAction("/tasks/", "addTask");
  type Route = Routes["/tasks/"];
...
function TasksList() {
  ...
  const deleteTaskAction = useAction("/tasks/", "deleteTask");
